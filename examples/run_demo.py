"""Compare standard and normalized conformal intervals on heteroskedastic data.

The point of the demo: on data whose noise grows across the input space, both
methods hit the same marginal coverage, but only the normalized one puts its
width where the uncertainty actually is. Marginal coverage alone cannot tell
the two apart -- the per-region breakdown can.
...
def main():
    ...
    lines = [
        ...
        "| --- | --- | --- |",
        f"| standard | {standard_report['coverage']:.1%} | "
        f"{standard_report['mean_width']:.3f} |",
        f"| normalized | {normalized_report['coverage']:.1%} | "
        f"{normalized_report['mean_width']:.3f} |",
        "",
        "Both meet the target. On this summary alone the two look equivalent.",
        "",
    ...
    for name, report in (("standard", standard_report), ("normalized", normalized_report)):
    ...
    standard_spread = max(
        row["coverage"] for row in standard_report["by_group"]
    ) - min(row["coverage"] for row in standard_report["by_group"])
    normalized_spread = max(
        row["coverage"] for row in normalized_report["by_group"]
    ) - min(row["coverage"] for row in normalized_report["by_group"])
    ...
    lines += [
        ...
        f"normalized {normalized_spread:.1%}.",
        "",
        ...
        "the target. The normalized interval scales with local noise, so each "
        "region is served at close to the rate asked for.",
        "",
